...
def run_and_log(model_name, model):
    with mlflow.start_run(run_name=model_name):
        ...
        avg_inference_time = inf_elapsed / max(len(X_test), 1)
        ...
        print(f"{model_name}: MSE={mse:.4f}, RMSE={rmse:.4f}, R2={r2:.4f}, avg_inf={avg_inference_time:.6f}s")
        return (model_name, model, mse, r2)

# Train two baseline models
results.append(run_and_log("LinearRegression", LinearRegression()))
results.append(run_and_log("DecisionTreeRegressor", DecisionTreeRegressor(max_depth=5)))

# Pick best by lowest MSE
best_model_name, best_model, best_mse, best_r2 = min(results, key=lambda x: x[2])
...
with mlflow.start_run(run_name=f"{best_model_name}-Best-Register"):
    mlflow.log_params({"selected_by": "mse", "best_mse": best_mse, "best_r2": best_r2})
    mlflow.sklearn.log_model(best_model, "model", registered_model_name="BestHousingModel")
