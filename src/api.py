...
# ----- Metrics -----
LATENCY = Histogram("inference_latency_seconds", "Prediction latency in seconds")
PREDICTIONS = Counter("predictions_total", "Total prediction requests", ["status"])
...
def predict(features: Features):
    with LATENCY.time():
        try:
            ...
            logging.info("Request=%s Prediction=%s", features.model_dump(), prediction)
            c.execute(
                "INSERT INTO requests (MedInc,HouseAge,AveRooms,AveBedrms,Population,AveOccup,Latitude,Longitude,Prediction) VALUES (?,?,?,?,?,?,?,?,?)",
